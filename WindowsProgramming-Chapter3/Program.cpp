...
Program::Program(HINSTANCE hInstance, const Vector2<LONG>& size)
...
void Program::SetGrid(const Vector2<int>& gridSize, int gridThickness,
...
void Program::Init(int nCmdShow)
{
...
void Program::Render()
{
	...
	if (_isDebug && _debugSelectedObject != nullptr)
	{
		if (_winSize.bottom > 140 && _winSize.right > 220) {
			...
			auto TrailVel = std::format(L"Trailing Velocity : {{{:.3f}, {:.3f}}}", debugInfo.trailingVelocity.x, debugInfo.trailingVelocity.y);
			TextOutW(_dibDC, _winSize.right - 200, _winSize.bottom - 90, TrailVel.c_str(), TrailVel.size());
			...
			for (int i = 0; i < posQueueSize; ++i)
			{
				...
				/*COLORREF pxColor = GetPixel(_backDC, _debugPosQueue[i].x, _debugPosQueue[i].y);
				int alphaR = (pxColor & 0xFF) * (posQueueSize - i)
					/ posQueueSize;
				int alphaG = ((pxColor >> 8) & 0xFF) * (posQueueSize - i)
					/ posQueueSize +
					255 * i / posQueueSize;
				int alphaB = ((pxColor >> 16) & 0xFF) * (posQueueSize - i)
					/ posQueueSize;
				COLORREF alphaColor = RGB(
...
LRESULT Program::WndProcEntry(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_CREATE)
	{
	...
	else if (Program* pThis = reinterpret_cast<Program*>(GetWindowLongPtrW(hWnd, GWLP_USERDATA)))
	{
...
void Program::ContactBorder(Object<float>& assignment1Obj)
{
...
void Program::DrawGrid(const Vector2<LONG>& regionSt)
{
